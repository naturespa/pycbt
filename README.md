...
情報I向けの自己完結型CBTです。HTML・CSS・JavaScriptだけで構成され、ブラウザで `index.html` を開くだけで動作します。追加サービス、API、データベース、ビルド処理は使用しません。
...
受験中データと提出済み情報はブラウザの `localStorage` に保存します。通信処理は行いません。結果はJSONファイルとしてダウンロードし、授業内で指定された方法により提出します。
